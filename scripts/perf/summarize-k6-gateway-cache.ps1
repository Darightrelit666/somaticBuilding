param(
  [string]$BeforeDir = "D:\somaticBuilding\docs\performance\gateway\before-cache",
  [string]$AfterDir = "D:\somaticBuilding\docs\performance\gateway\optimized",
  [string]$OutCsv = "D:\somaticBuilding\docs\performance\reports\gateway_exercise_list_cache_comparison.csv",
  [string]$OutMd = "D:\somaticBuilding\docs\performance\reports\gateway_exercise_list_cache_comparison.md"
)
...
foreach ($vus in $vusLevels) {
  $beforePath = Join-Path $BeforeDir ("gateway_exercise_list_v{0}_before.json" -f $vus)
  $afterPath = Join-Path $AfterDir ("gateway_exercise_list_v{0}_after.json" -f $vus)
